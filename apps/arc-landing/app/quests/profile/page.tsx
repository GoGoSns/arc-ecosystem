...
const RARITY_COLORS = {
  ...
  legendary: 'text-[#d4af37] border-[#d4af37]/20 bg-[#d4af37]/5 shadow-[0_0_15px_rgba(212, 175, 55,0.1)]',
};
    ...
    return (
      <main className="min-h-screen bg-[#050508] text-white flex flex-col items-center justify-center p-4">
        <div className="bracket-card p-12 text-center max-w-md">
          <Brackets />
          <div className="h-20 w-20 bg-[#d4af37]/10 rounded-full flex items-center justify-center mx-auto border border-[#d4af37]/20 mb-8">
            <Lock className="text-[#d4af37]" size={32} />
          </div>
          <h1 className="text-3xl font-black mb-4">WALLET REQUIRED</h1>
          <p className="text-[#555566] mb-8">Connect your wallet to view your quest profile, XP, and badges.</p>
          <button onClick={connect} className="bracket-button w-full justify-center py-4">CONNECT WALLET</button>
          <Link href="/quests" className="block mt-6 text-sm text-[#555566] hover:text-[#d4af37] transition-colors">
            Back to Quest Hub
          </Link>
        </div>
      </main>
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <p className="font-mono text-xs uppercase tracking-[0.2em] text-[#d4af37]">User Profile</p>
            <h1 className="text-4xl font-black mt-2">
              {address.slice(0, 6)}...{address.slice(-4)}
            </h1>
            <p className="text-[#555566] font-mono text-xs mt-2 uppercase tracking-widest">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_350px]">
          ...
          <div className="space-y-8">
            {/* Level & XP */}
            <div className="bracket-card p-10 bg-gradient-to-br from-[#d4af37]/5 to-transparent">
              <Brackets />
              <div className="flex flex-col md:flex-row items-center gap-12">
                ...
                <div className="relative h-48 w-48">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <p className="font-mono text-[10px] uppercase text-[#555566] tracking-[0.2em]">LEVEL</p>
                ...
                <div className="flex-1 w-full">
                  <div className="grid grid-cols-2 gap-8 mb-8">
                    <div>
                      <p className="font-mono text-[10px] uppercase text-[#555566] tracking-[0.2em] mb-1">Total XP</p>
                      <p className="text-4xl font-black text-[#d4af37]">{progress.totalXp}</p>
                    </div>
                    <div>
                      <p className="font-mono text-[10px] uppercase text-[#555566] tracking-[0.2em] mb-1">Rank</p>
                      <p className="text-4xl font-black">#12</p>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <div className="flex justify-between font-mono text-xs text-[#555566]">
                      <span>{progress.totalXp} XP</span>
                      <span>{getXpForLevel(progress.level + 1)} XP</span>
                    </div>
                    <div className="h-3 w-full bg-[#0d0d12] rounded-full overflow-hidden border border-white/5">
                      <div 
                        className="h-full bg-gradient-to-r from-[#d4af37] to-[#f0d78c]"
                        style={{ width: `${percent}%` }}
                      />
                    </div>
                    <p className="text-center font-mono text-[10px] text-[#555566] uppercase pt-2">
            ...
            <section>
              <h2 className="text-2xl font-black uppercase mb-6 flex items-center gap-3">
                <Medal className="text-[#d4af37]" /> BADGE COLLECTION
              </h2>
                ...
                {BADGES.map((badge) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-[#050508] opacity-0 group-hover:opacity-100 flex items-center justify-center p-2 text-[10px] leading-tight">
                        {badge.description}
                      </div>
                    </div>
                  );
            ...
            <section>
               <h2 className="text-2xl font-black uppercase mb-6 flex items-center gap-3">
                <Trophy className="text-[#d4af37]" /> ACHIEVEMENTS
              </h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {milestones.map((m) => (
                  <div key={m.label} className={`flex items-center gap-4 p-4 border rounded-xl ${m.reached ? 'border-[#d4af37]/30 bg-[#d4af37]/5' : 'border-[#1a1a2e] opacity-50'}`}>
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center border ${m.reached ? 'bg-[#d4af37] border-[#d4af37] text-black' : 'border-[#1a1a2e] text-[#1a1a2e]'}`}>
                      <CheckCircle2 size={20} />
                    </div>
                    <div>
                      <p className={`font-black uppercase text-sm ${m.reached ? 'text-white' : 'text-[#555566]'}`}>{m.label}</p>
          ...
          <div className="space-y-8">
             <div className="bracket-card p-6">
              ...
              <h3 className="text-sm font-black uppercase mb-6 flex items-center gap-2">
                <Clock className="text-[#d4af37]" size={16} /> QUEST HISTORY
              </h3>
              <div className="space-y-4">
                {completedQuests.length > 0 ? (
                  completedQuests.slice().reverse().map((q) => (
                    <div key={q.id} className="flex gap-3 border-l-2 border-[#d4af37]/30 pl-4 py-1">
                      <div>
                        <p className="text-xs font-black uppercase">{q.title}</p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] font-mono text-[#d4af37]">+{q.xpReward} XP</span>
                          <span className="text-[9px] font-mono text-[#555566]">&middot; COMPLETED</span>
                        </div>
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-xs text-[#555566] italic text-center py-8">No quests completed yet.</p>
                )}
              </div>
              {completedQuests.length > 5 && (
                <button className="w-full mt-6 text-[10px] font-mono uppercase text-[#555566] hover:text-white transition-colors">
                  View full history
                </button>
              )}
            </div>
            ...
            <div className="bracket-card p-6 bg-white/[0.02]">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-xs">
                  <span className="text-[#555566]">Completion Rate</span>
                  <span>{Math.round((progress.completedQuests.length / QUESTS.length) * 100)}%</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-[#555566]">Quests Left</span>
                  <span>{QUESTS.length - progress.completedQuests.length}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-[#555566]">Badges Found</span>
