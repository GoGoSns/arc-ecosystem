  ...
  return (
    <div className="min-h-screen bg-[#050508] text-white">
      ...
      <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        <p className="text-[#555566] text-sm mb-8">Apply now to validate when Arc mainnet launches.</p>

        {/* Info box */}
        <div className="rounded-2xl p-5 mb-6" style={{ background: 'rgba(212, 175, 55,0.06)', border: '1px solid rgba(212, 175, 55,0.2)' }}>
          <ul className="space-y-2 text-sm text-[#8a8a9a]">
            <li className="flex gap-2"><span className="text-[#d4af37]">›</span> Arc is currently on testnet with 10 active validators.</li>
            <li className="flex gap-2"><span className="text-[#d4af37]">›</span> Mainnet launch will gradually expand the validator set.</li>
            <li className="flex gap-2"><span className="text-[#d4af37]">›</span> Early applicants receive priority consideration.</li>
          </ul>
        </div>
        ...
        <div className="rounded-2xl p-6 mb-8" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <h2 className="text-sm font-black uppercase tracking-widest text-[#d4af37] mb-5">Your Application</h2>

          {!isConnected ? (
            <div className="text-center py-6">
              <p className="text-sm text-[#555566] mb-4">Connect your wallet to apply for the validator waitlist.</p>
            ...
            <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <div className="grid sm:grid-cols-2 gap-3 text-xs text-[#555566]">
                <div><span className="text-[#555566]">Region:</span> {REGION_LABELS[myApp.region]}</div>
                <div><span className="text-[#555566]">Applied:</span> {new Date(myApp.appliedAt).toLocaleDateString()}</div>
              </div>
              <p className="text-xs text-[#555566] mt-3 leading-relaxed">{myApp.experience.slice(0, 200)}{myApp.experience.length > 200 ? '...' : ''}</p>
            </div>
          ) : submitted ? (
            <div className="text-center py-4">
              <div className="text-4xl mb-3">✅</div>
              <p className="font-bold text-[#4ade80]">Application submitted!</p>
              <p className="text-xs text-[#555566] mt-1">We'll review your application and contact you.</p>
            </div>
          ) : (
            <div className="space-y-5 max-w-lg">
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8a8a9a] mb-1.5">Email (optional)</label>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8a8a9a] mb-1.5">
                ...
                <div className="flex justify-between mt-1">
                  ...
                  <span className="text-xs text-[#555566]">{experience.length} chars</span>
                </div>
              </div>
        ...
        {approved.length > 0 && (
          <div className="rounded-2xl p-6 mb-8" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <h2 className="text-sm font-black uppercase tracking-widest text-[#d4af37] mb-4">Approved Validators</h2>
            <div className="space-y-2">
              {approved.slice(0, 10).map((v) => (
                <div key={v.address} className="flex items-center justify-between px-3 py-2 rounded-lg"
                  style={{ background: 'rgba(255,255,255,0.02)' }}>
                  <span className="text-sm font-mono text-[#8a8a9a]">{v.name || shortenAddr(v.address)}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-[#555566]">{v.region}</span>
        ...
        {admin && validators.length > 0 && (
          <div className="rounded-2xl p-6" style={{ background: 'rgba(212, 175, 55,0.04)', border: '1px solid rgba(212, 175, 55,0.2)' }}>
            <h2 className="text-sm font-black uppercase tracking-widest text-[#d4af37] mb-4">Admin — Review Applications</h2>
              ...
              {validators.map((v) => (
                <div key={v.address} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 rounded-xl"
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#555566]">{REGION_LABELS[v.region]} · {new Date(v.appliedAt).toLocaleDateString()}</p>
                  </div>
