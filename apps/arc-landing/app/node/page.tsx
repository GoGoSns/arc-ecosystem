    ...
    <div className="min-h-screen bg-[#050508] text-white">
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[#555566] text-lg">Run a node. Secure the network. Earn rewards.</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-10">
          {[
            ...
            <div key={label} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(212, 175, 55,0.12)' }}>
              <div className="flex items-center gap-2 mb-2">
                <Icon size={13} className="text-[#d4af37]" />
                <span className="text-[10px] text-[#555566] uppercase tracking-wider">{label}</span>
              </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 mb-6">
          {/* Quick Setup */}
          <div className="rounded-2xl p-6 flex flex-col gap-4" style={{ background: 'rgba(212, 175, 55,0.06)', border: '1px solid rgba(212, 175, 55,0.2)' }}>
            <div>
              <div className="text-2xl mb-1">🚀</div>
              <h2 className="text-lg font-black mb-1">Quick Setup</h2>
              <p className="text-sm text-[#555566]">Get your node running in 15 minutes</p>
            </div>
            <div className="space-y-2">
              <Link href="/node/setup" className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold w-full" style={{ background: '#d4af37', color: '#0a0a0a' }}>
                View Setup Guide
              </Link>
              ...
              <button
                ...
                disabled
                aria-disabled="true"
                className="w-full rounded-lg border border-white/5 py-2 text-xs text-[#555566] transition-colors opacity-50 cursor-not-allowed"
              >
                Download docker-compose.yml
              </button>
              <p className="text-center text-[10px] uppercase tracking-[0.24em] text-[#1a1a2e]">Coming soon</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl p-6 flex flex-col gap-4" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div>
              ...
              <p className="text-sm text-[#555566]">Check RPC status, block height, network health</p>
            </div>
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)' }}>
              ...
              <span className="text-[10px] text-[#555566] ml-auto">rpc.testnet.arc.network</span>
            </div>
          ...
          <div className="rounded-2xl p-6 flex flex-col gap-4" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div>
              <Globe size={24} className="mb-2 text-[#d4af37]" />
              <h2 className="text-lg font-black mb-1">Node Map</h2>
              <p className="text-sm text-[#555566]">Explore operator coverage across regions</p>
            </div>
            <div className="text-sm text-[#8a8a9a]">
              Browse the live mock map with status colors, filters, and detailed operator info.
            </div>
            <Link href="/node/map" className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold w-full" style={{ background: '#d4af37', color: '#0a0a0a' }}>
              Open Node Map
            </Link>
          </div>
          ...
          <div className="rounded-2xl p-6 flex flex-col gap-4" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div>
              ...
              <p className="text-sm text-[#555566]">Apply now to become a mainnet validator</p>
            </div>
            <div className="text-sm text-[#8a8a9a]">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-10">
          ...
          <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <p className="text-xs text-[#555566] mb-3">Find bugs, earn USDC rewards. HackerOne integrated.</p>
            <p className="text-xs text-[#555566] mb-3">{openBugs} open {openBugs === 1 ? 'bounty' : 'bounties'}</p>
            <Link href="/node/bounty" className="text-xs font-bold text-[#d4af37] hover:underline">Browse Bounties →</Link>
          </div>
          ...
          <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <p className="text-xs text-[#555566] mb-3">Multiple RPC endpoints to choose from</p>
            <p className="text-xs text-[#555566] mb-3">{rpcProviders.length} {rpcProviders.length === 1 ? 'provider' : 'providers'} available</p>
            <Link href="/node/rpc" className="text-xs font-bold text-[#d4af37] hover:underline">View All →</Link>
          </div>
          ...
          <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <p className="text-xs text-[#555566] mb-3">Top operators by uptime</p>
            <div className="space-y-1 mb-3">
              {topOps.length === 0
                ? <p className="text-[10px] text-[#555566]">No operators yet</p>
                : topOps.map((op, i) => (
                    <div key={op.address} className="flex items-center gap-2 text-xs">
                      <span>{medals[i]}</span>
                      <span className="text-[#8a8a9a]">{op.name || shortenAddr(op.address)}</span>
                      <span className="ml-auto text-[#4ade80]">{op.uptimePercent ?? '—'}%</span>
                    </div>
                  ))}
            </div>
            <Link href="/node/leaderboard" className="text-xs font-bold text-[#d4af37] hover:underline">View Leaderboard →</Link>
          </div>
        </div>

        {/* System requirements */}
        <div className="rounded-2xl p-6 mb-8" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <h2 className="text-sm font-black uppercase tracking-widest text-[#d4af37] mb-4">System Requirements</h2>
          <div className="grid sm:grid-cols-2 gap-6">
            {[
              ...
              <div key={label}>
                <p className="text-xs font-bold text-[#8a8a9a] uppercase tracking-wider mb-2">{label}</p>
        ...
        {/* Quick links */}
        <div className="rounded-2xl p-6 mb-8" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <h2 className="text-sm font-black uppercase tracking-widest text-[#d4af37] mb-4">Quick Links</h2>
        ...
        <div className="rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(212, 175, 55,0.15)' }}>
          <h2 className="text-sm font-black uppercase tracking-widest text-[#d4af37] mb-4">Register as Operator</h2>

          {!isConnected ? (
            <div className="text-center py-6">
              <p className="text-sm text-[#555566] mb-4">Connect your wallet to register as a node operator.</p>
            ...
            <div className="rounded-xl p-4" style={{ background: 'rgba(74,222,128,0.06)', border: '1px solid rgba(74,222,128,0.2)' }}>
              ...
              <div className="grid sm:grid-cols-3 gap-3 text-sm">
                <div><span className="text-[#555566] text-xs">Address</span><div className="text-[#8a8a9a] font-mono text-xs mt-0.5">{shortenAddr(myOp.address)}</div></div>
                <div><span className="text-[#555566] text-xs">Region</span><div className="text-[#8a8a9a] mt-0.5">{REGION_LABELS[myOp.region]}</div></div>
                <div><span className="text-[#555566] text-xs">Status</span><div className="mt-0.5 capitalize" style={{ color: myOp.status === 'verified' ? '#4ade80' : myOp.status === 'inactive' ? '#ef4444' : '#facc15' }}>{myOp.status}</div></div>
              </div>
            </div>
          ) : regDone ? (
            <div className="text-center py-4">
              ...
              <p className="text-xs text-[#555566] mt-1">Your application is pending review.</p>
              <div className="flex gap-3 justify-center mt-4">
                <Link href="/forum/c/node" className="text-xs text-[#d4af37] hover:underline">Join Forum</Link>
                <Link href="/node/validator" className="text-xs text-[#d4af37] hover:underline">Apply for Validator</Link>
              </div>
            </div>
          ) : (
            <div className="space-y-4 max-w-lg">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8a8a9a] mb-1.5">Display Name (optional)</label>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8a8a9a] mb-1.5">RPC Endpoint (optional)</label>
                <input value={regRpc} onChange={(e) => setRegRpc(e.target.value)} placeholder="https://your-node:26657"
                  className="w-full rounded-xl px-4 py-2.5 text-sm text-white placeholder-[#555] outline-none"
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)' }} />
              </div>
              <button onClick={handleRegister} className="px-6 py-2.5 rounded-xl text-sm font-bold" style={{ background: '#d4af37', color: '#0a0a0a' }}>
                Register
              </button>
            </div>
          )}
